...
if __name__ == "__main__":
    ...
    theta_test = np.pi
    phi_test = np.pi/2
    xpos_test = rad_test*np.sin(theta_test)*np.sin(phi_test)
    ypos_test =rad_test*np.sin(theta_test)*np.cos(phi_test)
    zpos_test = rad_test*np.cos(theta_test)
